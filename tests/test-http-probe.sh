...
MOCK_CURL_CODE=200 MOCK_CURL_EXIT=0 "$ROOT/http-probe.sh" 8080 "$cookie" || fail "HTTP 200 was not accepted"
...
MOCK_CURL_CODE=302 MOCK_CURL_EXIT=0 "$ROOT/http-probe.sh" 8080 "$cookie" || fail "HTTP redirect was not accepted"

if MOCK_CURL_CODE=500 MOCK_CURL_EXIT=0 "$ROOT/http-probe.sh" 8080 "$cookie"; then
    fail "HTTP 500 was accepted"
fi

if MOCK_CURL_CODE=000 MOCK_CURL_EXIT=7 "$ROOT/http-probe.sh" 8080 "$cookie"; then
    fail "curl transport failure was accepted"
fi

if MOCK_CURL_CODE=200 MOCK_CURL_EXIT=0 "$ROOT/http-probe.sh" 9999 "$cookie" 2>/dev/null; then
